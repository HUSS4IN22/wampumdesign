...
export default function WampumBuilder() {
    ...
    useEffect(() => {
        if (gridRef.current) {
            ...
            gridRef.current.style.setProperty('--bead-gap', `${BEAD_GAP_PX}px`);
        }
    }, []); 
    ...
    return (
        <div className="w-full flex flex-col items-center bg-theme-light-bg text-theme-text px-8">
            ...
            <div className="w-full max-w-full overflow-x-auto p-2 border border-gray-300 rounded-lg bg-white shadow-lg">
                <div
                    className="wampum-grid grid gap-px border-2 border-gray-700 bg-gray-400 p-1.5 select-none"
                    ref={gridRef} 
                >
